...
#[derive(Clone)]
pub struct MethodUserRemoveBlacklist {
    pub table_symbol_flag: StrategyTable<SharedSledStorage, DbRowSymbolFlag>,
}
#[async_trait(?Send)]
impl RequestHandler for MethodUserRemoveBlacklist {
    type Request = build::model::UserRemoveBlacklistRequest;
    ...
    async fn handle(&self, ctx: RequestContext, req: Self::Request) -> Response<Self::Request> {
        ...
        for symbol_request in req.list {
            ...
            let rows = table.update_symbol_flag(asset._hash(), true).await;
            match rows {
        ...
        Ok(build::model::UserRemoveBlacklistResponse {
