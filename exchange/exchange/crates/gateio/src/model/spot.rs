...
use crate::model::order::GateioOrderStatus;

#[serde_as]
#[derive(Debug, Serialize, Deserialize)]
pub struct Balance {
    pub currency: Asset, // Asset

    #[serde_as(as = "DisplayFromStr")]
    pub available: f64, // Free
...
pub struct ExecutionReport {
    ...
    #[serde(rename = "X")]
    pub current_order_status: GateioOrderStatus, // Current order status
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(tag = "e")]
pub enum GateioSpotWebsocketData {
...
pub fn decode_gateio_spot_websocket_message(
    account: AccountId,
    exchange: Exchange,
    data: &str,
    manager: Option<SharedInstrumentManager>,
) -> Result<Option<ExecutionResponse>> {
    let data: GateioSpotWebsocketData = serde_json::from_str(data)?;
    match data {
        GateioSpotWebsocketData::AccountPosition(_) => Ok(None),
        GateioSpotWebsocketData::BalanceUpdate(update) => {
        ...
        GateioSpotWebsocketData::ExecutionReport(execution_report) => {
            ...
            let update_order = UpdateOrder {
                ...
                ty: execution_report.order_type.parse()?,
