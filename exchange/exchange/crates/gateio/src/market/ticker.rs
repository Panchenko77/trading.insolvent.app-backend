...
pub struct GateioBookTickerChannel {
    exchange: Exchange,
    manager: Option<SharedInstrumentManager>,
}

impl GateioBookTickerChannel {
    pub fn new(exchange: Exchange, manager: Option<SharedInstrumentManager>) -> Self {
        Self { exchange, manager }
    }

    pub fn parse_binance_book_ticker(&self, msg: GateioBookTicker, received_time: Time) -> eyre::Result<BookTicker> {
