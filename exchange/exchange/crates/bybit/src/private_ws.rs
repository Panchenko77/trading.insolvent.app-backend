use crate::model::parse_bybit_ws_message;
...
impl BybitPrivateWs {
    pub fn new(account: AccountId, url: BybitUrls, signing: SigningApiKeySecret) -> Self {
        Self {
            url,
            signing,
            ws: WsSession::new(),
    ...
    pub fn handle_ws_message(&mut self, message: Message) -> Option<ExecutionResponse> {
        if let Ok(text) = message.into_text() {
            if let Ok(response) = parse_bybit_ws_message(self.account, &text, None) {
